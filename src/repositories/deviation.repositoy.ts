...
export const getDeviations = async (operation?: string): Promise<Array<Deviation>> => {
    const repository = getRepository(Deviation);
    return repository.find({ operation: { id: operation } });
};
  
export const createDeviation = async (payload: IDeviationPayload): Promise<Deviation> => {
    const repository = getRepository(Deviation);
