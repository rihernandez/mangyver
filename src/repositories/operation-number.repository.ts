...
export const getOperationNumbers = async (operation?: string): Promise<Array<OperationNumber>> => {
    const repository = getRepository(OperationNumber);
    return repository.find({ operation: { id: operation } });
};
  
export const createOperationNumber = async (payload: IOperationNumberPayload): Promise<OperationNumber> => {
    const repository = getRepository(OperationNumber);
...
export const getOperationNumber = async (id: string): Promise<OperationNumber | null> => {
    const repository = getRepository(OperationNumber);
